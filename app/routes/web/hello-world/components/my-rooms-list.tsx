...
import { Clock, MapPin, Users, Edit, Trash2, MessageSquare } from "lucide-react"
import {
  AlertDialog,
  AlertDialogAction,
  AlertDialogCancel,
...
const mockHostRooms = [
  {
    ...
    isHost: true,
    restaurantInfo: {
      description: "Artisanal pizza with wood-fired oven and craft beer selection.",
...
export function MyRoomsList() {
  ...
  const handleLeaveRoom = (e:any, roomId:string) => {
    ...
    try {
      const joinedRooms = loadJoinedRooms().filter((room:any) => room.id !== roomId)
    ...
    toast(
      "Left room! You've successfully left the room."
    )
  }
  ...
  const handleDeleteRoom = () => {
    if (roomToDelete) {
      ...
      toast(
         "Room deleted Your room has been deleted successfully.",
      )
    }
  }
  ...
  const handleCommentClick = (e:any) => {
    e.stopPropagation() // Prevent card click
    toast(
       "Comment Restaurant review feature coming soon!",
    )
  }
  ...
  const handleEditClick = (e:any) => {
    e.stopPropagation() // Prevent card click
    toast(
       "Edit Edit room feature coming soon!",
    )
  }
  ...
  if (myRooms.length === 0) {
    return (
      <div className="text-center py-12">
        <h3 className="text-lg font-medium mb-2">You haven't joined any rooms yet</h3>
        <p className="text-muted-foreground mb-6">Join a room or create your own to get started</p>
          ...
          >
                ...
                <div className="flex flex-col items-center">
                  {room.host ? (
                    <Avatar>
                      ...
                      <AvatarFallback>{room.host.name?.charAt(0) || "U"}</AvatarFallback>
                    </Avatar>
                  ) : (
                    <Avatar>
                  ...
                  <span className="text-xs text-muted-foreground mt-1">{room.isHost ? "You" : "Host"}</span>
                </div>
              </div>
            </CardHeader>
            ...
            <CardFooter className="flex justify-between items-center pt-3 border-t">
              ...
              {room.isHost ? (
                ...
                <Button variant="destructive" onClick={(e) => handleLeaveRoom(e, room.id)}>
                  Leave Room
                </Button>
              )}
            </CardFooter>
      ...
      <AlertDialog open={!!roomToDelete} onOpenChange={() => setRoomToDelete(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This will permanently delete this room and remove all participants. This action cannot be undone.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
